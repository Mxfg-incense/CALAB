...
void simd_matmul(void) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < p; j+=4)
        {
            __m128 cij = _mm_setzero_ps();
            for (int k = 0; k < 4; ++k)
            {
                __m128 aik = _mm_set1_ps(a[i][k]);
                __m128 bkj = _mm_loadu_ps(b[k] + j);
                cij = _mm_add_ps(cij, _mm_mul_ps(aik, bkj));
            }
            _mm_storeu_ps(c2[i] + j, cij);
        }
    }
    clock_gettime(CLOCK_MONOTONIC, &end);
    printf("simd: %f\n", (float) (end.tv_sec - start.tv_sec) + (float) (end.tv_nsec - start.tv_nsec) / 1000000000.0f);
...
void loop_unroll_simd_matmul(void) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < p; j+=4)
        {
            __m128 cij = _mm_setzero_ps();
            __m128 aik, bkj;
            aik = _mm_set1_ps(a[i][0]);
            bkj = _mm_loadu_ps(b[0] + j);
            cij = _mm_add_ps(cij, _mm_mul_ps(aik, bkj));

            aik = _mm_set1_ps(a[i][1]);
            bkj = _mm_loadu_ps(b[1] + j);
            cij = _mm_add_ps(cij, _mm_mul_ps(aik, bkj));

            aik = _mm_set1_ps(a[i][2]);
            bkj = _mm_loadu_ps(b[2] + j);
            cij = _mm_add_ps(cij, _mm_mul_ps(aik, bkj));

            aik = _mm_set1_ps(a[i][3]);
            bkj = _mm_loadu_ps(b[3] + j);
            cij = _mm_add_ps(cij, _mm_mul_ps(aik, bkj));
